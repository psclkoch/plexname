...
```
$ plexname inception
  🔍 Inception (2010) — Film — mit Leonardo DiCaprio, Joseph Gordon-Levitt
     Korrekt? (Enter/n):
  ✅ tt1375666 übernommen.
✅ Erstellt: Inception (2010) {imdb-tt1375666}

$ plexname breaking bad
  🔍 Breaking Bad (2008) — Serie — mit Bryan Cranston, Aaron Paul
     Korrekt? (Enter/n):
  📺 Staffeln anlegen (TMDB: 5, Enter = übernehmen):
  ✅ tmdb-1396 übernommen.
✅ Erstellt: Breaking Bad (2008) {tmdb-1396}
   ✅ Season 01
   ✅ Season 02
   ✅ Season 03
   ✅ Season 04
...
```
$ plexname
Noch nicht eingerichtet. Starte Setup...

==================================================
  🎬 plexname — Ersteinrichtung
==================================================

1) OMDb API Key (für Film-Lookups via IMDb-ID)
   Kostenlos erstellen: https://www.omdbapi.com/apikey.aspx
   → 'FREE' wählen, E-Mail eingeben, Key kommt per Mail.
   API Key: ________

2) TMDB Read Access Token (für Titelsuche, Serien, Cast)
   Account erstellen: https://www.themoviedb.org/signup
   Token holen: https://www.themoviedb.org/settings/api
   → Den langen 'API-Token für Lesezugriff' kopieren (nicht den kurzen API-Schlüssel).
   Token: ________

3) Plex-Filmordner (Stammverzeichnis der Filmbibliothek)
   Beispiel: /Volumes/NAS/Filme oder /mnt/media/movies
   Pfad: ________

4) Plex-Serienordner (Stammverzeichnis der Serienbibliothek)
   Beispiel: /Volumes/NAS/Serien oder /mnt/media/tv
   Pfad: ________

✅ Konfiguration gespeichert: ~/.config/plexname/config.json
```
